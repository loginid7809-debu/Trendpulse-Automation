...
class Config:
    """Central configuration for TrendPulse automation."""
    ...
    THUMB_PALETTES = [
        [(180, 0,   0),   (255, 120, 0)],    # Red → Orange
        [(0,   40,  180), (0,   180, 255)],   # Blue → Cyan
        [(100, 0,   180), (255, 0,   120)],   # Purple → Pink
        [(0,   120, 0),   (0,   220, 100)],   # Green
        [(255, 180, 0),   (255, 80,  0)],     # Yellow → Orange
        [(20,  20,  20),  (60,  60,  60)],    # Dark dramatic
    ]
    ...
    # ── Royalty-free Music URLs (Pixabay CDN - no account needed) ──
    MUSIC_URLS = [
    ...
    # ─────────────────────────────────────────────────────────
    @classmethod
    def random_voice(cls, language: str = 'en') -> str:
    ...
    @classmethod
    def ensure_dirs(cls):
        for path in [cls.TEMP_DIR, cls.TEMP_ASSETS,
                     cls.DATA_DIR, cls.FONTS_DIR]:
            os.makedirs(path, exist_ok=True)

    @classmethod
    def yt_category(cls, name: str) -> str:
